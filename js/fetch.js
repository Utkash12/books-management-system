...
class BookCategoryManager {
    ...
    async fetchBooksData() {
        try {
            ...
            const response = await fetch(`https://openlibrary.org/subjects/${this.categorySelect.value}.json`);
    ...
    populateBooksTable() {
        this.books.forEach(book => {
            ...
            authorCell.innerText = book.authors && book.authors[0] ? book.authors[0].name : 'Unknown Author';
            isbnCell.innerText = book.cover_id || 'No Cover';
            publishYearCell.innerText = book.first_publish_year ? book.first_publish_year.toString() : 'N/A';
            genreCell.innerText = this.categorySelect.value;
            editionCountCell.innerText = book.edition_count ? book.edition_count.toString() : 'N/A';
            discountCell.innerText = this.calculateDiscount(book.edition_count || 0).toString();
    ...
    filterBooksByCategory() {
        ...
        rows.forEach(row => {
            ...
            const shouldShow = rowCategory === this.categorySelect.value || this.categorySelect.value === 'all';
            if (shouldShow) {
